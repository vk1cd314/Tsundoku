...
class MangaChapter(val parent: NetworkCaller<JSONObject>, private val chapterId: String) :
    ...
    override fun onCallSuccess(result: JSONObject?) {
        try {
            ...
            val ret = JSONObject()
            val urlPref = "${result.getString("baseUrl")}/data/${
                result.getJSONObject("chapter").getString("hash")
            }"
